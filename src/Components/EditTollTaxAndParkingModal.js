...
function EditTollTaxAndParkingModal(props) {
  ...
  const _openCamera = async (setState, item) => {
    ...
    try {
      ...
      if (getCompImg) {
        props.setselectedDoc(getCompImg);
        props.setselectedDocName(getCompImg);
      } else {
        props.setselectedDoc(res?.path);
...
export default memo(EditTollTaxAndParkingModal);
